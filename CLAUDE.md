...
- Recepção em nuvem, processamento local: webhooks (Meta, Komune) em Edge Functions → filas `pgmq`; workers consomem quando ligados (ADR-04). **Atualizado em 14/09/2026:** o worker-wa roda no Fly.io (`infra/nuvem/fly.worker-wa.toml`), sempre ligado; ingest, IA e rotas continuam na máquina dedicada.
- WhatsApp: Cloud API oficial da Meta, conexão direta. Nunca Baileys, Evolution ou qualquer automação não oficial (ADR-06). **Atualizado em 14/09/2026:** sem Coexistence — o número da KOMUNE fica só na Cloud API e o time inteiro atende pelo CRM, cada mensagem assinada com o primeiro nome de quem envia (guia em `docs/operacao/whatsapp-no-crm.md`). O PRD ainda descreve Coexistence e eco do celular.
