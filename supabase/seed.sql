...
-- =====================================================================
-- 12c. Roteiro de ligação em árvore — MORA NA MIGRAÇÃO, não aqui
--
--     A árvore de 60 nós vive em
--     `supabase/migrations/20260909180000_o_roteiro_para_de_vender_casamento.sql`,
--     e não neste arquivo, por uma razão que já custou caro uma vez.
--
--     Até 09/09/2026 ela estava nos DOIS lugares: a seed semeava a v1 e, quando um
--     defeito aparecia, a correção tinha de ser escrita duas vezes — uma na seed,
--     para o `db reset`, e outra numa migração `do $$`, para o banco que já estava
--     de pé. O comentário da migração 20260904001500 diz isso com todas as letras:
--     "A seed já foi corrigida no arquivo; este bloco conserta o roteiro que JÁ
--     está no banco, porque supabase/seed.sql só roda em db reset".
--
--     Duas cópias do mesmo texto divergem — é o que acontece com duas cópias de
--     qualquer coisa. E este texto em particular é lido em voz alta para um
--     estranho: divergir aqui significa a Heloísa dizendo uma frase que ninguém
--     revisou.
--
--     Como a migração roda ANTES da seed no `db reset`, o banco novo já nasce com
--     a versão publicada. A autoverificação do bloco 13 confere a árvore
--     publicada, seja ela qual for, em vez de cravar um número de versão.
-- =====================================================================
...
begin
  ...
  select jsonb_array_length(r.arvore), cardinality(app.validar_roteiro(r.arvore))
    ...
    from public.call_scripts r where r.slug = 'captacao_v1' and r.is_published;
  ...
  -- Piso, e não igualdade: a árvore cresce a cada versão (37 nós na v1, 60 na v2),
  -- e travar o número exato transformaria toda reescrita de roteiro numa edição da
  -- seed. O que importa aqui é que ela exista e não tenha erro estrutural — quem
  -- confere o conteúdo é `roteiro-publicado.test.ts`, com o validador de verdade.
  if n_nos < 30 then raise exception 'seed: roteiro de ligação com só % nós', n_nos; end if;
  if n_rot_err > 0 then raise exception 'seed: roteiro de ligação com % erro(s) estrutural(is): %',
    n_rot_err, array_to_string((select app.validar_roteiro(r.arvore) from public.call_scripts r
                                 where r.slug = 'captacao_v1' and r.is_published), ' '); end if;
