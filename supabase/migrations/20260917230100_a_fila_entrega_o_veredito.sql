-- =====================================================================
-- A fila entrega o veredito da IA junto com a pontuação
--
-- A leitura do modelo (migração 20260917230000) grava `ia_veredito` e
-- `ia_porque` no candidato, e a fila não os devolvia — o dado existia e a tela
-- não via, que é o mesmo lugar onde a pontuação estava de manhã.
--
-- Terceira vez hoje que esta função é recriada a partir de
-- `pg_get_functiondef`. Continua valendo o motivo: são 100 linhas com
-- mascaramento de PII por papel, busca por CNPJ e detecção de duplicata.
-- DROP + CREATE porque o Postgres não deixa `create or replace` mudar a lista de
-- colunas devolvidas; os grants voltam iguais.
-- =====================================================================
...
comment on function public.radar_fila(text,int,int,text,boolean,int,int) is
  'A fila de revisão do Radar (RF-RAD-11), ordenada pela triagem (RF-RAD-12). Devolve pontuacao, faixa (A+..C) e o veredito da IA sobre o nome — que é opinião, não decisão. Telefone e e-mail saem mascarados para quem não lê PII da base.';
