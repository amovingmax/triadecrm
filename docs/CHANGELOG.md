...
**Descoberta 1 — `pg_net` e `pg_cron` não existiam nos projetos.** O `komune-crm` não tinha `pg_net`, então `app.komune_push_disparar` falhava com `schema "net" does not exist`; o `komune-dev` não tinha `pg_cron`, e por isso o bloco de agendamento da migração das 13h foi **pulado** pelo próprio `if exists` (que era o comportamento desenhado, mas escondia o buraco). As duas foram criadas à mão.

**A consequência que eu temi não existe, e foi verificada em vez de suposta:** os 14 agendamentos do `komune-crm` estão registrados e ativos (`aplicar_retencao`, `cadencias_agendar`, `dlq_drenar`, `komune_push`, `recompute_temperatures`, `relatorio_semanal`, `wa_confirmacoes_reenfileirar` e os outros sete). O `pg_cron` já existia ali; **só o `pg_net` faltava** — e é exatamente por isso que os agendamentos nasceram bem enquanto o disparo morria em `schema "net" does not exist`. A extensão que faltava não era a que agenda, era a que telefona.

**Efeito colateral a decidir:** com `push_ativo = true` e o job `komune_push` de 5 em 5 minutos, o `komune-crm` — que é o projeto de PRODUÇÃO do Tríade — passa a empurrar todo pré-cadastro novo para o `komune-dev`, porque é para lá que `komune_push_url` aponta. Enquanto a integração estiver em desenvolvimento isso é o desejado; no dia em que alguém captar um fornecedor de verdade, não é.
...
## D10 — 08/09/2026 — A interface deixa de falar por código, e a tela para de sobrar (PRD §8; ADR-01)

Queixa do Matheus, em duas frases: "to achando muito confuso esse CRM e feio também" e "muita informação, muita poluição, telas mal aproveitadas, com espaço sobrando". As duas viraram mudanças concretas; nenhuma regra de negócio foi tocada.

- **A base sai do azul lavado** (commit anterior, `25638c7`): `--background` era `#f0f8ff` (aliceblue) e a rampa `--grafite-*` inteira era tingida de ardósia. 18 valores trocados por uma rampa neutra de verdade, com o contraste conferido antes: tinta 9,61 → 9,75; esmaecido 4,76 → 4,93; borda 3,23 → 3,20 (mínimo 3,0). Nenhum papel regrediu. Poppins → Geist e IBM Plex Mono → Geist Mono.
- **A barra lateral também sai do azul**: `--sidebar` era `#e0f2fe`, a única superfície cromática da interface, e puxava o olho para a navegação — que é o que menos importa na tela. Agora é `--grafite-100`, um degrau acima do fundo: separa por valor, não por cor. `--sidebar-muted-foreground` recalculado para `#5c5c64` (5,87:1 na lateral, 4,98:1 no realce de passagem, 6,19:1 na barra inferior).
- **O número deixa de ser monoespaçado**: o utilitário `numerico` chamava `var(--font-mono)`, e o zero cortado do Geist Mono, num contador de 2xl, era lido como defeito da tela. Ficou só `tabular-nums`, que é o que faz a coluna alinhar — o Geist tem algarismo de largura fixa.
- **Os códigos de requisito saem da interface**: 44 ocorrências varridas em `apps/web/src`. 35 eram parêntese de referência e saíram por corte; 9 estavam no meio da oração e foram reescritas ("o RF-CON-24 pede no máximo isso por turno" → "o limite por turno é este"). Saíram também a pílula `RF-REL-09, RF-REL-01` do cabeçalho dos oito painéis de Relatórios (e o campo `requisitos`, que não é mais renderizado) e o "chega no D8" da paleta de comandos: dia de calendário é metadado de roadmap.
- **O mesmo corte no banco** (`20260908160000_o_relatorio_deixa_de_falar_por_codigo.sql`): o texto do relatório de segunda é lido no celular e às vezes colado num grupo, e carregava sete referências ("a fórmula do RF-REL-10", "a meta de densidade do RF-REL-03"). `app.relatorio_semanal_catalogo()`, `app.relatorio_semanal_fatos(date)` e `app.frase_variacao_semanal(jsonb)` voltam inteiras por `create or replace` — só o texto muda, a assinatura e a lógica são idênticas (conferido por diff linha a linha contra a migração original). Relatórios já gerados não mudam: o texto está guardado em `weekly_reports`, que é o ponto do desenho; quem quiser o texto novo numa semana antiga usa "Refazer com os dados de agora".
- **A explicação nasce fechada** (`components/ui/nota-recolhida.tsx`): os blocos "O que ainda não está ligado" (Agenda e Rota) e "O que ainda não entra nesta fila" (Meu dia) ocupavam meia tela cada, sempre abertos, empurrando a lista de trabalho para baixo da dobra. Viraram `<details>` nativo: abre sem JavaScript, o Ctrl+F do navegador expande sozinho ao encontrar e o leitor de tela anuncia o estado.
- **A coluna se centraliza**: cada tela escolhia a própria largura e todas colavam na barra lateral — num monitor de 1440px sobrava um terço de vazio à direita, que é a queixa literal do "espaço sobrando". A casca ganhou `mx-auto w-full max-w-[1440px]` e as sete telas com teto próprio passaram a se centralizar dentro dele. Teto também protege o ultrawide: nenhuma tabela se estica por 2500px.
- **A faixa de indicadores do Meu dia**: quatro caixas com moldura viraram uma faixa só, dividida por régua fina — elevação não comunica hierarquia nenhuma ali, porque as quatro métricas têm o mesmo peso. O "sem meta" repetido quatro vezes saiu: a frase logo abaixo da faixa já diz isso uma vez e explica o que fazer.
- **A busca de Parceiros**: três linhas (rótulo, caixa, lista de formatos) para uma pergunta só. A lista de formatos virou o placeholder, o rótulo virou `sr-only` e o `aria-describedby` continua apontando para a lista, que segue no documento.
- Verificado: lint, typecheck, 530 testes Vitest e build (23 rotas) verdes; migração aplicada e revertida em transação no Postgres local (`CREATE FUNCTION` ×3, sem erro); telas Meu dia, Parceiros, Funis e Relatórios percorridas no navegador em 1440×900, sem código de requisito visível e sem erro de console.
- Pendente: aplicar a migração no `komune-crm` em nuvem; publicar no Vercel para o time ver; o mesmo tratamento de densidade nas telas de Conversas, Radar, Cadências, Metas e Admin, que ainda não foram revisadas nesta passada.

## D10 — 08/09/2026 — As cinco telas que faltavam, e o jargão que sobrava (PRD §8)

Segunda passada do redesenho, nas telas que a primeira não alcançou: Conversas, Radar, Cadências, Metas e Admin.

- **O jargão de anexo sai da tela**: além dos `RF-XXX-NN` da passada anterior, apareciam 14 referências a anexos, ADRs e ao próprio PRD em texto que a pessoa lê ("Pelo R13, o primeiro contato é por LIGAÇÃO", "o R08 manda uma pessoa olhar", "as 11 fontes avaliadas no anexo R03", "por decisão (ADR-05)"). Todas reescritas em português; nenhuma perdeu conteúdo — "Pelo R13" virou "por decisão do projeto", "PRD §10.6, apagar dados manter hash e data" virou "apaga o número, guarda o hash e a data".
- **A decisão do modo automático sai do texto, não do dado**: `app_settings['cadencia.modo_automatico'].decisao` guarda `"ADR-05"` e continua guardando — é a rastreabilidade da decisão. O que mudou é que a tela parou de imprimir o código entre parênteses.
- **Cem eventos de sistema no banco diziam "pesquisa R09"**: as 100 fichas importadas da lista-semente carregavam essa frase no corpo da atividade, visível na linha do tempo de cada parceiro. Trocada por "pesquisa de mercado" (100 linhas, `type = 'system'` e `outcome_id is null`, então o único gatilho que dispara em `update` retorna sem tocar em negócio nenhum — conferido antes). `scripts/seed-leads-100.sql` corrigido junto, para uma reimportação não reintroduzir.
- **Mais duas buscas em três linhas**: Conversas e Radar repetiam o padrão rótulo/caixa/formatos que já havia sido corrigido em Parceiros. Mesma solução: formatos no placeholder, rótulo em `sr-only`, `aria-describedby` intacto.
- **Conversas parou de dizer a mesma coisa duas vezes**: sem fio de WhatsApp, a caixa da janela de 24 h ("Nunca escreveram para a gente") ficava empilhada com a caixa que explica o caminho e oferece a ação ("Não há conversa de WhatsApp com este parceiro"). A primeira só aparece quando há fio — aí ela decide o que pode sair e quando.
- **Radar**: a regra de comportamento do coletor virou `NotaRecolhida`, e os dois painéis saíram do `max-w-3xl` que abria um buraco à direita enquanto a fila embaixo ia até a borda.
- **Cadências**: os quatro tetos por canal viraram uma faixa dividida por régua, o mesmo formato da faixa de indicadores do Meu dia.
- **Metas**: com uma pessoa só (o que o sdr enxerga), o cartão encostava à esquerda e sobrava metade da tela — agora se centraliza. E o "sem meta" que aparecia três vezes no mesmo cartão virou uma: a frase de rodapé some quando não há meta, porque a etiqueta ao lado do número e o aviso do topo já dizem isso.
- Verificado: lint, typecheck, 530 testes e build verdes; Conversas, Radar, Cadências, Metas e Admin percorridas no navegador em 1440×900.
- Pendente: a tela de Admin só foi vista como `sem-permissao` no ambiente local (a sessão de desenvolvimento é sdr); conferir em produção, onde a conta é admin.

## D4 — 08/09/2026 — A coleta do Radar cabe num botão (RF-RAD-01, RF-RAD-02; ADR-04, ADR-08)

Buraco encontrado na revisão de "o que falta para usar": a esteira de ingestão existe inteira desde o D4 e o catálogo de coleta desde o D5, mas **nada, em lugar nenhum do produto, enfileirava um `ingest_jobs`**. `esteira_abrir_lote` só cria a linha em `import_batches`, porque quem a chama é a importação de planilha — e planilha continua passando pela prévia por desenho. Para pedir uma coleta era preciso montar o payload da fila à mão e chamar `esteira_fila_enfileirar` por psql com service_role; foi assim que os 42 cerimonialistas entraram na base em 08/09. Isso não é operável por quem usa o CRM.

- **`public.radar_coletar_agora(int, text[], int)`** (`20260908170000`): abre o lote e enfileira o job **na mesma transação**. As duas juntas ou nenhuma — um lote aberto sem job na fila fica em `previa` para sempre, que foi exatamente o que aconteceu com o lote de teste. Sete recusas nomeadas, todas antes de qualquer escrita: `sem_permissao`, `origem_invalida`, `origem_desabilitada`, `coletor_desligado`, `sem_catalogo`, `categoria_fora_do_catalogo` (devolvendo a lista do que existe, para a pessoa escolher sem sair da tela) e `ja_rodando`.
- **`ja_rodando` não é conveniência, é o limite por fonte**: duas coletas simultâneas na mesma fonte dobram o tráfego contra o intervalo que o `robots.txt` já fixou. O que o R03 permite é uma corrida, não duas.
- **O lote de coleta nasce em `na_fila`, não em `previa`**: coleta não tem prévia, o que se pede é "vá buscar". A prévia continua sendo o caminho da planilha.
- **A resposta diz se o coletor está de pé** (`worker_heartbeats`, janela de 5 min). É a diferença entre "o pedido entrou" e "os dados vêm": com a máquina desligada o lote espera na fila, e a tela avisa em vez de deixar a pessoa achar que falhou.
- **Botão "Coletar" na aba Fontes do Radar**: só aparece onde pode dar certo — quem pode ligar fonte, fonte ligada, robô escrito e catálogo com caminho. Hoje isso é verdade só para o Casamentos.com.br. Botão que a RPC recusaria de qualquer jeito é promessa, e a pessoa só descobriria depois do clique.
...
## D7 — 08/09/2026 — A agenda do Google entra no CRM (RF-AGE-02 parcial, RF-AGE-04)

Era o último item do PRD sem uma linha escrita, e o Rafael pediu em 08/09/2026. Até aqui, uma reunião marcada vivia só dentro do Tríade: ninguém do outro lado recebia convite, não havia link do Meet e nenhum lembrete tocava. Alguém repetia o compromisso à mão na própria agenda.

**O que passa a funcionar:** botão "Pôr na agenda" em cada compromisso com hora marcada (e em visita). Ele cria o evento na agenda da pessoa, com sala do Meet, convite para o fornecedor e lembretes de 24 h e 1 h. Depois de criado, o botão vira "Entrar no Meet".

- **Duas tabelas em dois schemas, e essa separação é o ponto do desenho.** `app.agendas_do_google` guarda a LIGAÇÃO da pessoa com a conta Google dela — uma credencial de longa duração que abre a agenda de alguém. Mora no schema `app`, que o PostgREST não expõe, com RLS ligada e zero políticas: nenhum papel do PostgREST lê ou escreve. E o token nem fica na linha: vai para o **Vault**, e a tabela guarda só o id do segredo. `public.compromissos_no_google` guarda o RESULTADO (id do evento, link do Meet), que é dado de trabalho e a tela precisa mostrar — em `public`, com RLS que segue a visibilidade da tarefa em vez de reescrever a regra.
- **Vault e não coluna `text`**, pela razão prática: um dump do banco com refresh tokens em texto puro é um arquivo que abre a agenda de seis pessoas.
- **Conectar é um login de novo, e a tela avisa antes.** O Supabase entrega o `provider_refresh_token` só no retorno de um consentimento pedido com `access_type=offline` e `prompt=consent`; não há "só mais essa permissão". A pessoa precisa escolher a MESMA conta, porque escolher outra troca a sessão do CRM — o texto do painel diz isso antes do clique.
- **A rota `/api/agenda/evento` checa acesso com o cliente DA PESSOA** (RLS de `tasks` respondendo) e só então usa o cliente de serviço para alcançar o Vault. Regra de acesso duplicada é regra que diverge.
- **Grava o espelho depois do Google, nunca antes.** Gravar primeiro deixaria o CRM afirmando um evento que talvez não exista, com link do Meet apontando para o nada. O único desencontro possível é o inverso — evento criado e registro falhou — e ele tem recado próprio ("foi criado na sua agenda, confira antes de tentar de novo"), porque um "tente de novo" genérico ali cria evento duplicado.
- **Cada recusa tem dono.** `api_desabilitada` → Luiz habilita a API; `acesso_revogado` → a pessoa reconecta (e a ligação é marcada como revogada, para a tela oferecer reconectar em vez de repetir o erro); `escopo_insuficiente` → a pessoa reconecta marcando a permissão; `nao_configurado` → falta variável no servidor.
- **Escopo mínimo:** `calendar.events` (cria e altera eventos), não `calendar` (lê a agenda inteira). O CRM não precisa ver compromisso particular de ninguém.
- **`timeZone: America/Fortaleza` explícito** no evento: a Vercel roda em UTC, e uma reunião de 14h viraria 11h no relógio de quem vai.
- **O botão não aparece em tarefa `a_marcar`.** Metade das tarefas `meeting` da base é "Marcar apresentação", cuja hora é prazo calculado e não hora combinada com ninguém — pô-las no calendário encheria a agenda de compromissos falsos. É a mesma distinção que o cabeçalho de `agenda/tipos.ts` protege desde o D7.
- **Desconectar APAGA o segredo do Vault** e mantém a linha, para a tela dizer "reconecte" em vez de "nunca conectou".
- Verificado: 21 asserções pgTAP (`supabase/tests/38_a_agenda_do_google.sql`), das quais 8 provam que o token não sai (authenticated não lê a tabela nem executa a função do token; `agenda_google_estado` não devolve o segredo em campo nenhum apesar de poder lê-lo; desconectar apaga do Vault); 533 testes Vitest; lint, typecheck e build verdes com as duas rotas registradas; migração aplicada no `komune-crm` em nuvem.
- **Pendente, e bloqueia o uso:** três chaves fora do repositório — habilitar a Google Calendar API, declarar o escopo na tela de consentimento e pôr `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET` e `SUPABASE_SERVICE_ROLE_KEY` na Vercel. Passo a passo em `docs/operacao/ligar-a-agenda-do-google.md`.
- **Fora do escopo desta entrega:** sugerir horário livre (a outra metade do RF-AGE-02). Exige o escopo de LEITURA da agenda, bem mais invasivo, e ninguém pediu.

## D7 — 09/09/2026 — O evento acompanha o reagendamento (RF-AGE-04)

Buraco na entrega de ontem, encontrado relendo o caminho de "Reagendar" a pedido do Rafael. **O evento no Google ficava no horário velho.**

O motivo está em `move_deal`: reagendar não altera a tarefa da reunião — ela é FECHADA e uma tarefa NOVA é inserida com a data combinada. O espelho em `compromissos_no_google` é chaveado por `task_id`, então continuava apontando para a tarefa fechada, e a tarefa nova nascia sem evento. O efeito era o pior possível e invisível do nosso lado: o fornecedor ficava com um convite para um horário que não valia mais, enquanto no CRM a reunião nova aparecia certinha.

- **PATCH no evento, não apagar e criar.** O Google tem semântica própria: alterar avisa o convidado de que a reunião FOI REMARCADA, com a mesma sala do Meet e o mesmo fio de e-mail. Apagar e criar manda um cancelamento seguido de um convite novo — duas notificações, dois links, e a impressão de que a reunião caiu.
- **O espelho muda de dono.** `app.compromisso_do_google_remanejar` acha a tarefa nova por (mesmo negócio, kind `meeting`, `due_at` exatamente no horário combinado, ainda aberta) — os quatro valores que o próprio `move_deal` acabou de escrever. `move_deal` não devolve o id dessa tarefa (só o da próxima ação), então não havia como recebê-lo do cliente.
- **Não achou, não mexe.** Mover o espelho para a tarefa errada é pior que deixá-lo parado: a reunião certa passaria a mostrar o link de outra. Mesma regra quando a tarefa nova já tem evento próprio — aí existem dois no Google, e quem escolhe qual apagar é gente.
- **Google primeiro, espelho depois.** Invertendo, um PATCH que falhasse deixaria o CRM mostrando a reunião nova como "já está na agenda" com link para o horário ERRADO — mentira invisível. Nesta ordem a falha possível é o inverso, visível e corrigível.
- **O token é o de QUEM CRIOU o evento** (`app.agenda_google_token_do_evento`), e não o de quem clicou. O evento vive na agenda de quem o criou; a Heloísa remarcando uma reunião da Bárbara receberia 404 do Google com o token dela.
- **Botão "Tirar da agenda"** para o que o remanejamento não cobre: reunião cancelada de vez, evento posto por engano, ou os dois eventos que sobram de um remanejamento interrompido. `sendUpdates=all` no DELETE — cancelamento silencioso é pior que nenhum, porque o fornecedor aparece.
- **O aviso sobe para a tela** quando o registro entra mas o Google fica para trás. `warning` e não `error`, porque o reagendamento está gravado; e com 12 s de duração, porque é a única falha desta tela que ninguém da Komune veria sozinho.
...
**Decisão humana / pendente:**
**A escala entrou no guardrail de PII (decisão do Rafael, 17/09).** A auditoria foi calibrada para ler *uma* mensagem por chamada: janela sem fronteira nenhuma, qualquer corrida de 10 a 13 dígitos começando por DDD válido. A ficha lê a conversa inteira, e conversa de fornecedor é data, horário, preço e quantidade — colados pela junção, viram telefone. Medido no corpus de 40 mensagens reais do repositório, nenhuma com telefone: **31 de 31 conversas de dez mensagens eram barradas**; o guardrail decidia o produto em vez de proteger o dado. Agora o prompt declara `escala: 'conversa'` e é auditado pela varredura com fronteira de letra (atravessa pontuação, espaço, hífen e emoji; não atravessa palavra) — 0 de 31 barradas. Quem não declara escala continua na varredura estrita, e os quatro prompts antigos não mudaram uma linha. `camposDoTriade` passou a aceitar caminho aninhado (`mensagens[].quando`), e só quando a raiz é texto de fora. **O preço, em lista literal no eval:** de oito grafias de telefone de verdade, a varredura da conversa deixa de pegar uma — com palavra entre os grupos de dígitos (`é 84 depois 9 9988 depois 0011`) —, que é a mesma única que a regra da Anatel também não mascara. A regra de pseudonimização não mudou em nada.

**Provado ponta a ponta:** `workers ai --chamada-de-teste` contra o dublê local, com o cliente e o SDK reais — `ai_runs` 71, entrada 131, saída 182, leitura de cache 739, custo contabilizado e saída validada pelo schema.
...
Correção na revisão: as duas funções que preenchem `{{origem}}` chamam-se `app.envio_campos_da_ficha` e `public.wa_preparar_envio`. A spec (§3.3 e nota de rodapé 3) e o plano da Fase 1 as chamam de `app.envio_variaveis` e `app.wa_preparar_abertura` — nomes que não existem no banco — e o comentário da migração tinha copiado os errados. Os arquivos e as linhas citados sempre estiveram certos, e o motivo do `kind = 'import'` continua de pé; só os nomes mudaram.
...
7. **a corrida de verdade**, que o pgTAP não consegue provar: duas sessões `psql` lado a lado, `begin` nas duas, `public.reuniao_marcar(...)` no mesmo horário do mesmo dono, `commit` na primeira — a segunda destrava e devolve `ok: false` com três alternativas. (No pgTAP isso não se prova: `dblink` de dentro da transação é deadlock que o Postgres não detecta, porque o detector enxerga locks e não sockets.) **Conferido em 25/09** — e o motivo que volta é `horario_indisponivel`, não `horario_tomado`: o advisory lock de (pessoa, dia) serializa as duas antes do índice, então quem chega depois é barrado na reconferência da grade. `horario_tomado` é o ramo de baixo, para quem escrever em `reunioes` por fora desse lock; os dois devolvem alternativas no mesmo retorno.
...
### Conferência da Fase 5 (25/09/2026) — e o opt-out que recusava gritando

A fase foi conferida commit a commit, com a suíte inteira reconstruída do zero e três provas que o pgTAP não alcança, feitas em sessões `psql` concorrentes de verdade.

**O que se confirmou.** A trava é do índice, não do código: duas transações inserindo `14h00–14h40` e `14h20–15h00` na agenda da mesma pessoa — a segunda bloqueia, espera o commit da primeira e morre com `23P01` sobre `reunioes_sem_colisao`; e `14h40–15h20`, que **encosta**, entra, porque o intervalo é `[)`. Pelo caminho de produto, duas chamadas concorrentes a `public.reuniao_marcar` no mesmo horário gravam **uma** reunião e a outra volta com três alternativas. A grade devolve exatamente nove começos (9h30 → 16h10, o último terminando 16h50), zero em fim de semana, zero no dia em que se insere um feriado, zero dentro das próximas 3 h, zero além dos 10 dias úteis; com `route_plans` no dia o último começo oferecido é 12h50; com quatro reuniões a grade fecha, com três sobram seis, e reunião **cancelada** não segura horário nem conta no teto. `9h29`, `17h00` e `17h21` não existem na grade e são recusados ao marcar. O e-mail falhando não desmarca nada: cinco voltas com backoff de 30 s a 240 s, dead-letter na quinta, e a reunião segue viva, com o eco em `tasks` e `aviso_enviado_em` nulo — que é justamente o que o cartão lê para dizer que ninguém foi avisado. A faxina do Google não deixou função, tabela, invólucro, rota, arquivo nem segredo de Vault órfãos, e o login do CRM (`SUPABASE_AUTH_GOOGLE_*`) e a chave do Maps continuam onde estavam. Nada foi a produção, nada foi empurrado, e a árvore está limpa.

**O que estava errado, e foi corrigido.** `app.reuniao_gravar` conferia o opt-out lendo `organizations.do_not_contact` na mão. Isso deixava de fora os outros dois caminhos do guardrail: o **contato** que pediu para sair (`contacts.do_not_contact`) e o **telefone** que caiu na `suppression_list` pela regra de palavras. Nesses dois a reunião não chegava a existir — o gatilho `app.tasks_guard_suppressed` derrubava a transação inteira ao inserir a `tasks`-eco —, mas derrubava **levantando exceção**: o robô receberia um 500 do PostgREST em vez de `motivo = 'suprimido'`, e a tela diria "não deu para falar com o servidor" a quem tinha acabado de pedir para sair. O guardrail segurava; a recusa é que não falava a língua de quem chama.

A conferência passa a usar `app.is_suppressed_target(v_org, v_contato)`, o **mesmo árbitro** que `app.pode_tocar` usa na ordem 1 (ADR-03: uma regra, um lugar). Os três caminhos agora devolvem `{"ok": false, "motivo": "suprimido"}` antes de qualquer escrita. Quatro asserções novas no `74_a_reuniao.sql` cobrem os três caminhos e provam que nenhum deles deixa reunião para trás.
